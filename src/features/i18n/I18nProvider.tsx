...
interface I18nContextValue {
  ...
  tf: (key: TranslationKey, params: Record<string, string>) => string;
}
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  function tf(key: TranslationKey, params: Record<string, string>): string {
    return translations[locale][key].replace(/\{(\w+)\}/g, (_, name) => params[name] ?? `{${name}}`);
  }
