...
export function GoalsPage() {
  ...
  async function handleContribute(goalId: string) {
    const raw = window.prompt("Quanto você quer guardar para essa meta agora?");
    if (!raw) return;
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-xl font-semibold text-ink-900">Metas</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {goals.length === 0 ? (
          <p className="text-sm text-ink-900/50">Nenhuma meta criada ainda.</p>
        ) : (
          goals.map((g) => {
            const progress = calculateGoalProgress(g, goalContributions);
            return (
              <div key={g.id} className="rounded-2xl border border-black/5 bg-white p-5">
                ...
                >
                  {progress.isComplete ? "Meta concluída 🎉" : "Guardar dinheiro"}
                </button>
              </div>
            );
          })
